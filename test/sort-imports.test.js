...
const { sortImports, classifyImport, extractPath, deduplicate } = require("../src/sort-imports");
...
describe("extractPath", () => {
  it("extracts double-quoted path from a bare import", () => {
    assert.equal(extractPath('import "@openzeppelin/contracts/access/Ownable.sol";'), "@openzeppelin/contracts/access/Ownable.sol");
  });

  it("extracts single-quoted path", () => {
    assert.equal(extractPath("import '../utils/Math.sol';"), "../utils/Math.sol");
  });

  it("extracts path from a named import", () => {
    assert.equal(extractPath('import { Ownable } from "@openzeppelin/contracts/access/Ownable.sol";'), "@openzeppelin/contracts/access/Ownable.sol");
  });
...
describe("classifyImport", () => {
  ...
  it("classifies third-party @openzeppelin as group 1", () => {
    assert.equal(classifyImport("@openzeppelin/contracts/access/Ownable.sol", scope), 1);
  });

  it("classifies third-party @chainlink as group 1", () => {
    assert.equal(classifyImport("@chainlink/contracts/src/v0.8/interfaces/AggregatorV3Interface.sol", scope), 1);
  });

  it("classifies @balancer-labs path with /interfaces/ as group 2", () => {
    assert.equal(classifyImport("@balancer-labs/v3-interfaces/contracts/vault/IVault.sol", scope), 2);
  });

  it("classifies @balancer-labs path without /interfaces/ as group 3", () => {
    assert.equal(classifyImport("@balancer-labs/v3-solidity-utils/contracts/math/FixedPoint.sol", scope), 3);
  });

  it("classifies ./ relative import as group 4", () => {
    assert.equal(classifyImport("./tokens/ERC20Base.sol", scope), 4);
  });

  it("classifies ../ relative import as group 4", () => {
    assert.equal(classifyImport("../shared/utils/MathLib.sol", scope), 4);
  });

  it("respects a custom first-party scope", () => {
    assert.equal(classifyImport("@my-org/core/interfaces/IFoo.sol", "@my-org"), 2);
    assert.equal(classifyImport("@my-org/core/lib/Foo.sol", "@my-org"), 3);
    assert.equal(classifyImport("@openzeppelin/contracts/Foo.sol", "@my-org"), 1);
  });
...
describe("deduplicate", () => {
  it("removes second occurrence of the same path", () => {
    ...
    assert.deepEqual(result.map((c) => c.path), ["A.sol", "B.sol"]);
  });
...
`;
    ...
    // longest path first
    assert.ok(lines[0].includes("ERC20Burnable"), `expected ERC20Burnable first, got: ${lines[0]}`);
    assert.ok(lines[1].includes("IERC20"), `expected IERC20 second, got: ${lines[1]}`);
    assert.ok(lines[2].includes("Ownable"), `expected Ownable third, got: ${lines[2]}`);
  });
...
`;
    ...
    const result = sortImports(input, SCOPE);
    const lines = result.split("\n").filter((l) => l.startsWith("import"));
    assert.ok(lines[0].includes("ERC165"), `expected ERC165 first (alphabetical), got: ${lines[0]}`);
    assert.ok(lines[1].includes("ERC777"), `expected ERC777 second, got: ${lines[1]}`);
  });
...
`;
    ...
    const blankLines = beforeContract.split("\n").filter((l) => l === "").length;
...
`;
    const result = sortImports(input, SCOPE);
    assert.ok(result.includes('import { ERC20 } from "@openzeppelin/contracts/token/ERC20/ERC20.sol";'));
    assert.ok(result.includes('import { Ownable } from "@openzeppelin/contracts/access/Ownable.sol";'));
  });
...
`;
    ...
    const lines = result.split("\n").filter((l) => l.startsWith("import"));
    assert.equal(lines[0], 'import "@openzeppelin/contracts/access/Ownable.sol";', "group 1 first");
    assert.equal(lines[1], 'import "@my-org/core/interfaces/IFoo.sol";', "group 2 (interface) second");
    assert.equal(lines[2], 'import "@my-org/core/lib/Helpers.sol";', "group 3 (first-party) third");
...
`;
    ...
    const ownable = result.split("\n").findIndex((l) => l.includes('import "@openzeppelin'));
    assert.equal(ownable, commentLine + 1, "comment is immediately above import");
  });
});
...
`;
    ...
    assert.ok(result.includes("IReClammPoolExtension"), "multi-line import must not be dropped");
...
`;
    ...
    assert.ok(fixedPointIdx < reClammMathIdx, "first-party group before relative group");
  });
...
`;
    ...
    assert.ok(result.includes("IReClammPoolExtension"), "multi-line import present");
    ...
    assert.ok(result.includes("ReClammPoolParams"), "ReClammPoolParams present");
    ...
    assert.ok(firstPartyIdx < relativeInterfaceIdx, "relative interface stays in group 4, after first-party");
  });
});

describe("relative interface classification", () => {
  it("keeps relative interface imports in group 4, not group 2", () => {
    assert.equal(classifyImport("./interfaces/IFoo.sol", "@balancer-labs"), 4);
    assert.equal(classifyImport("../interfaces/IBar.sol", "@balancer-labs"), 4);
  });
...
`;
    ...
    assert.ok(vaultIdx < fooIdx, "first-party interface before relative interface");
  });
  ...
  it("extractPath correctly handles multi-line import with quoted symbol names", () => {
    ...
    assert.equal(extractPath(multiLine), "./interfaces/IReClammPoolExtension.sol");
  });
});
...
describe("bare imports sort after specific imports within a group", () => {
  ...
  it("detects a bare import as non-specific", () => {
    assert.equal(isSpecificImport('import "@balancer-labs/v3-interfaces/contracts/vault/VaultTypes.sol";'), false);
  });

  it("detects named imports as specific", () => {
    assert.equal(isSpecificImport('import { IVault } from "@balancer-labs/v3-interfaces/contracts/vault/IVault.sol";'), true);
...
`;
    ...
    assert.ok(lines[lines.length - 1].includes("VaultTypes"), `expected VaultTypes last, got: ${lines[lines.length - 1]}`);
    // Specific imports come first
    assert.ok(lines[0].includes("IVaultErrors") || lines[0].includes("IHooks") || lines[0].includes("IVault"), "specific import first");
  });
...
`;
    ...
    const iVaultIdx = lines.findIndex((l) => l.includes('"IVault"') || l.includes("IVault.sol"));
    ...
    const reClammPoolMainIdx = lines.findIndex((l) => l.includes("IReClammPoolMain"));
    assert.ok(reClammPoolMainIdx < reClammMathIdx, "relative interface imports before lib imports");
  });
});